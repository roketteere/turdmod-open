# Starts SCUMServer.exe via turdmod-launcher with the same args the Manager
# Engine page uses. Triggers a UAC elevation prompt because SCUMServer.exe
...
if ($running -match "SCUMServer\.exe") {
    Write-Host "[$ScriptName] SCUMServer.exe ALREADY RUNNING. Run stop-server.ps1 first."
    exit 1
}
...
if ($taskExists) {
    ...
    $launcherArgs = @(
        ...
        "--skip-safety-check",
        # Everything after `--` is forwarded by the launcher to SCUMServer.exe.
        "--",
        "-nullrhi"
    )
...
# Confirm SCUMServer.exe appeared in the process list.
for ($i = 0; $i -lt 20; $i++) {
    Start-Sleep -Milliseconds 500
    $check = & tasklist /FI "IMAGENAME eq SCUMServer.exe" /NH 2>$null
    if ($check -match "SCUMServer\.exe") {
        Write-Host "[$ScriptName] OK - SCUMServer.exe is running."
        exit 0
    }
}

Write-Host "[$ScriptName] WARN - launcher OK but SCUMServer.exe not in tasklist after 10s. May have crashed during boot. Check UE4SS.log and SCUM.log."
exit 0

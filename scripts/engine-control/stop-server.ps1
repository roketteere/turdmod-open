# Stops GameServer.exe. Cross-elevation safe: uses taskkill /F which works
...
if (-not ($running -match "SCUMServer\.exe")) {
    Write-Host "[$ScriptName] GameServer.exe not running."
    exit 0
}

Write-Host "[$ScriptName] GameServer.exe is running. Calling taskkill /F..."
& taskkill /F /IM GameServer.exe 2>&1 | ForEach-Object { Write-Host "  $_" }
